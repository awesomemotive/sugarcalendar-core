 ...
 * @since 2.0.0
 */
class Week_Table extends Base_List_Table {
	...
	protected function display_mode() {
		...
		// Set start day & loop day
		$start_day = $day = date_i18n( 'd', $this->grid_start );
		...
		for ( $i = 0; $i < ( $this->day_count * 24 ); $i++ ) {
			...
			$this->set_current_cell( array(
				'start'  => mktime( $row, 0,  0, $this->month, $day, $this->year ),
				'end'    => mktime( $row, 59, 0, $this->month, $day, $this->year ),
				'row'    => $row,
				'index'  => $i,
				'offset' => 0
			) );
