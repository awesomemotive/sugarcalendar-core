 ...
 * @return array
 */
function sugar_calendar_taxonomy_args( $args = array(), $taxonomy = '' ) {
 ...
 * @return array
 */
function sugar_calendar_checklist_args( $args = array() ) {
	if ( ! empty( $args['taxonomy'] ) && ( sugar_calendar_get_calendar_taxonomy_id() === $args['taxonomy'] ) ) {
		...
		$r = apply_filters( 'sugar_calendar_checklist_args', array(
			'walker' => new Sugar_Calendar\Walker_Category_Radio()
		), $args );
 ...
 * @since 2.0.0
*/
function sugar_calendar_meta_box_add() {

	// Sections
	add_meta_box(
		'sugar_calendar_event_details',
		esc_html__( 'Event', 'sugar-calendar' ),
		'sugar_calendar_meta_box',
	...
	add_meta_box(
		...
		'sugar_calendar_details_metabox',
 ...
 * @param WP_Post $post
 */
function sugar_calendar_meta_box( $post = null ) {

	// Initialize the meta box
	$meta_box = new Sugar_Calendar\Meta_Box();
...
*/
function sugar_calendar_details_metabox( $post = null ) {
 ...
 * @return int
 */
function sugar_calendar_adjust_hour_for_meridiem( $hour = 0, $meridiem = 'am' ) {
	...
	// Filter & return
	return (int) apply_filters( 'sugar_calendar_adjust_hour_for_meridiem', $new_hour, $hour, $meridiem );
}
 ...
 * @return boolean
 */
function sugar_calendar_can_save_meta_box( $object_id = 0, $object = null ) {

	// Default return value
	$retval = false;

	// Bail if no nonce or nonce check fails
	if ( empty( $_POST['sugar_calendar_meta_box_nonce'] ) || ! wp_verify_nonce( $_POST['sugar_calendar_meta_box_nonce'], 'sugar_calendar_nonce' ) ) {
		return $retval;
	}
 ...
 * @return int|void
 */
function sugar_calendar_meta_box_save( $object_id = 0, $object = null ) {

	// Bail if meta-box cannot be saved
	if ( ! sugar_calendar_can_save_meta_box( $object_id, $object ) ) {
		return $object_id;
	}
	...
	// Get calendar date
	$date = ! empty( $_POST['sugar_calendar_date'] )
		? strtotime( sanitize_text_field( $_POST['sugar_calendar_date'] ) )
		: current_time( 'timestamp' );

	// Hour
	$hour = ! empty( $_POST['sugar_calendar_time_hour'] )
		? sanitize_text_field( $_POST['sugar_calendar_time_hour'] )
		: 0;

	// Minutes
	$minutes = ! empty( $_POST['sugar_calendar_time_minute'] )
		? sanitize_text_field( $_POST['sugar_calendar_time_minute'] )
		: 0;

	// Day/night
	$am_pm = ! empty( $_POST['sugar_calendar_time_am_pm'] )
		? sanitize_text_field( $_POST['sugar_calendar_time_am_pm'] )
		: 'am';

	/** Ends ******************************************************************/

	// Calendar date is set
	$end_date = ! empty( $_POST['sugar_calendar_end_date'] )
		? strtotime( sanitize_text_field( $_POST['sugar_calendar_end_date'] ) )
		: null;

	// Hour
	$end_hour = ! empty( $_POST['sugar_calendar_end_time_hour'] )
		? sanitize_text_field( $_POST['sugar_calendar_end_time_hour'] )
		: 0;

	// Minutes
	$end_minutes = ! empty( $_POST['sugar_calendar_end_time_minute'] )
		? sanitize_text_field( $_POST['sugar_calendar_end_time_minute'] )
		: 0;

	// Day/night
	$end_am_pm = ! empty( $_POST['sugar_calendar_end_time_am_pm']  )
		? sanitize_text_field( $_POST['sugar_calendar_end_time_am_pm'] )
		: 'am';

	/** All Day ***************************************************************/

	// Get all-day status
	$all_day = ! empty( $_POST['sugar_calendar_all_day'] )
		? (bool) $_POST['sugar_calendar_all_day']
		: false;
	...
	// Maybe tweak hours
	$hour     = sugar_calendar_adjust_hour_for_meridiem( $hour,     $am_pm     );
	$end_hour = sugar_calendar_adjust_hour_for_meridiem( $end_hour, $end_am_pm );
	...
	// Repeat
	$repeat = ! empty( $_POST['sugar_calendar_repeat'] )
		? sanitize_key( $_POST['sugar_calendar_repeat'] )
		: '';

	// Expire
	$expire = ! empty( $_POST['sugar_calendar_expire'] )
		? sanitize_text_field( $_POST['sugar_calendar_expire'] )
		: '';
	...
	if ( ! empty( $repeat ) && ! empty( $expire ) ) {
		...
		$recur_end_minutes = 0;
		$recur_end_am_pm   = 'am';
		$recur_end_hour    = sugar_calendar_adjust_hour_for_meridiem( $recur_end_hour, $recur_end_am_pm );
 ...
 * @return array
 */
function sugar_calendar_add_location_to_save( $event = array() ) {

	// Get event location
	$event['location'] = ! empty( $_POST['sugar_calendar_location'] )
		? wp_kses( $_POST['sugar_calendar_location'], array() )
		: '';
 ...
 * @return array
 */
function sugar_calendar_add_color_to_save( $event = array() ) {
 ...
 * }
 */
function sugar_calendar_calendar_meta_box( $post, $box ) {
